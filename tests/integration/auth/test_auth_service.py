...
async def test_google_auth__login_not_exist_user(auth_service, db_session):
    ...
    users = (await db_session.execute(select(UserProfile))).scalars().all()

    user = await auth_service.google_auth(code)

    assert len(users) == 0
    assert user is not None
    login_user = (await db_session.execute(select(UserProfile).where(UserProfile.id == user.user_id))).scalars().first()
