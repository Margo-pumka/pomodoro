...
from schema import UserLoginSchema, UserCreateSchema, GoogleUserData, YandexUserData
...
class AuthService:
    ...
    def generate_access_token(self, user_id: int) -> str:
        expires_date_unix = (dt.datetime.now(datetime.UTC) + dt.timedelta(days=7)).timestamp()
        token = jwt.encode(
    ...
    async def google_auth(self, code: str):
        user_data: GoogleUserData = await self.google_client.get_user_info(code)
    ...
    async def yandex_auth(self, code: str):
        user_data: YandexUserData = await self.yandex_client.get_user_info(code)
